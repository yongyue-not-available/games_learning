...
public:
	...
	void on_draw(const Camera& camera) override {
		...
		if (is_sun_text_active) {
			...
			text_position.x = position.x - (size.x - frame->getwidth()) / 2.0f;
	...
	void on_attack() {
		// Implement the attack logic for SunflowerPlayer
		Bullet* bullet = new SunBullet();

		Vector2 bullet_position;
		const Vector2& bullet_size = bullet->get_size();
		bullet_position.x = position.x + (size.x - bullet_size.x) / 2.0f;
	...
	void on_attack_ex() {
		// Implement the special attack logic for SunflowerPlayer
		is_attack_ex = true;
		is_sun_text_active = true;
		...
		bullet_position.x = target_position.x + (target_size.x - bullet_size.x) / 2.0f;
		bullet_position.y = -size.y;
		bullet_velocity.x = 0.0f;
		bullet_velocity.y = ex_bullet_speed;
